...
def configure_callback(conf):
    ...
    BAMBOO_URL = "http://" +  BAMBOO_HOST + ":" +str(BAMBOO_PORT) + "/confighash"
...
# gets the config-hash and convert it to 16bit int to use collectd as carrier
# collectd default datat types can not send string directly

def fetch_stats():

    try:
        result = urllib2.urlopen(BAMBOO_URL, timeout=10)
        confighash = str(int(result.read(), 16))
        return dispatch_stat(confighash)
...
def dispatch_stat(result):
    ...
    val = collectd.Values(plugin='bamboo_confighash')
    val.type = estype
